...
const getEmployeeOverFifty = (employeeData) => new Promise((resolve, reject) => {
  ...
  employeeData = employeeData.filter(empl=> empl.employeeAge > 50);
  resolve(employeeData.length);
})
...
const getTotalNoOfDaysSinceJoining = (employeeData) => new Promise((resolve, reject) => {
  ...
  let data = employeeData.map(emp => Math.floor((new Date().getTime() - new Date(emp.dateOfJoining).getTime()) / (1000 * 3600 * 24)));
  resolve(data);
}
