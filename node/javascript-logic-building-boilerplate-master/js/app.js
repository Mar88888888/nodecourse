...
{
       data.sort((detail1, detail2) => {
            const nameA = detail1.name.toUpperCase();
            const nameB = detail2.name.toUpperCase(); 
            if (nameA < nameB) {
            return -1;
            }
            if (nameA > nameB) {
...
const displayEmp = empDetails.map((emp)=>{
    emp = `${emp.id} ---- ${emp.name} ---- ${emp.salary} ---- ${emp.Bonus} ---- ${emp.appraisal}`
})
...
{
    if(maxappraisalscore == undefined || maxappraisalscore <=85){
        return 'Unfortunately, you are not eligible for a hike';
    }
    if(maxappraisalscore >= 100){
        return '20% hike to be given';
    }
    if( maxappraisalscore > 90 && maxappraisalscore < 100){
        return '10% hike to be given';
    }
    if(maxappraisalscore > 85 && maxappraisalscore <= 90){
